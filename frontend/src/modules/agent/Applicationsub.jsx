            ...
            <Card.Body>
              <Row className="align-items-center">
                <Col md={8}>
                  ...
                  <div className="text-muted small">{app.loanType} • {app.loanAmount}</div>
                  <div className="text-muted small mb-2">Mobile: {app.mobile}</div>
                  <div>
                    <span className="fw-semibold small">Documents:</span>
                    <div className="mt-1">
                      {app.documents.map((doc, index) => (
                        <Badge key={index} bg="secondary" className="me-1">{doc}</Badge>
                      ))}
                    </div>
                  </div>
                </Col>
                <Col md={4} className="text-md-end mt-3 mt-md-0">
                  {app.status === "Pending" ? (
                    <>
                      <Button size="sm" variant="success" className="me-2" onClick={() => updateStatus(app.id, "Accepted")}>
                        Accept
                      </Button>
